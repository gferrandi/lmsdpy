...
def augmented_GG(G, g):
    ll = G.shape[1]+1
    GG = np.zeros((ll,ll))
    GG[:-1,:-1] = G.transpose() @ G
    GG[:-1,-1] = (G.transpose() @ g).reshape(-1)
    GG[-1,:-1] = GG[:-1,-1]
    GG[-1,-1] = np.dot(g,g)
    return(GG)
...
def reduced_hessian_quad(G, g, mem, option, stepsize, tol_lin_dep, my_fun):
    if option == 'svd':
        u, s, v = la.svd(G, full_matrices=False) 
        v = v.transpose()
        ...
        AU = my_fun(v.transpose() * s.reshape(-1,1), u.transpose() @ g) * mem.reshape(1,-1)
        B = AU @ (v / s.reshape(1,-1))
        B = 0.5*(B + B.transpose())
        ...
        AU = my_fun(R1[:len(P),P1], Q.transpose() @ g) * mem.reshape(1,-1)
        B = np.linalg.solve(R.transpose(), AU[:,P].transpose())
        B = 0.5*(B + B.transpose())
        ...
        # [R | r] @ J @ R_inv
        small_r = np.linalg.solve(R.transpose(), G[:,P].transpose() @ g) 
        AU = my_fun(R, small_r) * mem[P].reshape(1,-1) 
        B = np.linalg.solve(R.transpose(), AU.transpose())
        ...
        if P.shape[0] > 2:
            ...
            AU = (np.linalg.solve(R[:-1,:-1].transpose(), AU.transpose())).transpose()
            ...
            T = np.tril(T,0) + np.tril(T,-1).transpose()
            
            # Q.T @ A**2 @ Q = T.T @ T + csi @ csi.T
            # find (Q.T @ A @ Q)_inv @ Q.T @ A**2 @ Q
            B = np.linalg.solve(T,  T.transpose() @ T + np.outer(csi, csi))
